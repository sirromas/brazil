...
function core_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    $node = new core_user\output\myprofile\node('contact', 'gender', 'Gender', null, null, $user->gender);
    $tree->add_node($node);

    $node = new core_user\output\myprofile\node('contact', 'birth', 'Data de Nascimento', null, null, $user->birth);
    $tree->add_node($node);

    $node = new core_user\output\myprofile\node('contact', 'cpf', 'CPF', null, null, $user->cpf);
    $tree->add_node($node);

    $node = new core_user\output\myprofile\node('contact', 'rg', 'RG', null, null, $user->rg . "&nbsp;" . $user->uf);
    $tree->add_node($node);

    $node = new core_user\output\myprofile\node('contact', 'license', 'Nr. CNH', null, null, $user->license);
    $tree->add_node($node);

    $node = new core_user\output\myprofile\node('contact', 'category', 'Categoria', null, null, $user->category);
    $tree->add_node($node);

    $node = new core_user\output\myprofile\node('contact', 'code', 'Formulário Renach', null, null, $user->code);
    $tree->add_node($node);
    ...
    if ($status == 1) {
        ...
        $node = new core_user\output\myprofile\node('contact', 'certificate', 'Certificado', null, null, 'N/A');
    } // end else
    $tree->add_node($node);
    ...
    if (empty($course) && ($interests = core_tag_tag::get_item_tags('core', 'user', $user->id))) {
        $node = new core_user\output\myprofile\node('contact', 'interests', get_string('interests'), null, null, $OUTPUT->tag_list($interests, ''));
        $tree->add_node($node);
    }
