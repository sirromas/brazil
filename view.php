...
require_once $_SERVER['DOCUMENT_ROOT'] . '/custom/dashboard/classes/Dashboard.php';
...
// *********** Custom code related to course access ***************

$ds = new Dashboard();
$ds_user = $ds->get_user_data();

if ($ds_user->userid != 2 && $ds_user->roleid == 5) {

    // 1. Check user payment status
    $paid_status = $ds->is_user_paid($ds_user->userid, $ds_user->courseid);
    if ($paid_status == 0) {
        $message = $ds->get_non_paid_message($ds_user);
        echo $message;
        die();
    }

    //2. Check user score status
    $grades = $ds->get_user_taken_quizes($ds_user->userid, $ds_user->courseid);
    if ($grades) {
        foreach ($grades as $gradeitem) {
            if ($gradeitem->rawgrade < $ds->passing_grade) {
                // Get paypal payments after failed test attempt
                $user_payment = $ds->is_user_paid($ds_user->userid, $ds_user->courseid, $gradeitem->timecreated);
                if ($user_payment == 0) {
                    $message = $ds->get_low_score_message($ds_user);
                    echo $message;
                    die();
                }
            }
    ...
    //3. Check user certification status
    $cert_status = $ds->is_user_passed($ds_user->userid, $ds_user->courseid);
    if ($cert_status == 1) {
        $ds->create_user_certificate($ds_user->userid, $ds_user->courseid);
        $message = $ds->get_certificate_message($ds_user);
        echo $message;
    }
} //end if $ds_user->userid != 2 && $ds_user->roleid == 5
